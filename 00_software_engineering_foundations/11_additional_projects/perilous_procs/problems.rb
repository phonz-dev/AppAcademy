...
# For example, this means that the array that corresponds to the key 2 should contain the elements 
# that return true when passed into the second proc.
# If an element returns true for multiple procs, then it should only be placed into the array that 
# corresponds to the proc that appears first in the arguments.
...
def proctition_platinum(arr, *procs)
  ...
  procs.each.with_index do |prc, i|
    selected = arr.select { |el| prc.call(el) }
    value = selected.reject { |el| seen.include?(el) }
    seen.concat(value)
    procs_result[i + 1] = value
  end
...
def procipher(sentence, procs)
  ...
  new_words = words.map do |word|
    new_word = word
    procs.each do |prc_key, prc_value|
      if prc_key.call(word)
        new_word = prc_value.call(new_word)
      end
    end
    new_word
  end
...
# contains procs as both keys and values. The method should return a new sentence where each 
# word of the input sentence is changed by a value proc if the original word returns true when 
# passed into the key proc. If an original word returns true for multiple key procs, then only 
# the value proc that appears earliest in the hash should be applied.
